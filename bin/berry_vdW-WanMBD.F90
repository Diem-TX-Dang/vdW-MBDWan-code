...
!------------------------------------------------------------!
! Modified by Diem TX Dang, Dai-Nam Le and Lilia M. Woods    !
!         University of South Florida, USA                   !
! Calculate polarizability in imaginary frequency Eq. (8)    !
!                used for vdW-WanMBD                         !
! All of modifications are noted as vdW-WanMBD               !
!------------------------------------------------------------!
module w90_berry
  ...
  public :: berry_main, berry_get_imf_klist, berry_get_imfgh_klist, berry_get_sc_klist, &
            berry_get_shc_klist, berry_get_polar_klist
...
contains
  ...
  subroutine berry_main
	...
	! vdW-WanMBD method: export polarizability alpha
	complex(kind=dp), allocatable :: polar_k_list(:, :, :)
    complex(kind=dp), allocatable :: polar_list(:, :, :)
    ...
    real(kind=dp)     :: kweight, kweight_adpt, kpt(3), kpt_ad(3), &
                         db1, db2, db3, fac, fac1, freq, rdum, vdum(3)
    ...
    eval_sc = .false.
    eval_shc = .false.
	!eval_polar = .false. ! vdW-WanMBD method
    ...
    if (eval_polar) then
      call get_HH_R
      call get_AA_R
	  allocate (polar_k_list(3, 3, kubo_nfreq))
      allocate (polar_list(3, 3, kubo_nfreq))
      polar_k_list = cmplx_0
      polar_list = cmplx_0
    endif
    ...
    if (on_root) then
      ...
      ! vdW-WanMBD method	  
      if (eval_polar) write (stdout, '(/,3x,a)') &
        '* Polarizability'
    ...
    if (wanint_kpoint_file) then
      ...
      do loop_xyz = 1, num_int_kpts_on_node(my_node_id)
        ...
        ! vdW-WanMBD method
        if (eval_polar) then
          call berry_get_polar_klist(kpt, polar_k_list)
		  polar_list = polar_list + polar_k_list*kweight
        end if
        !
	...
	! vdW-WanMBD method
	if (eval_polar) then
      call comms_reduce(polar_list(1, 1, 1), 3*3*kubo_nfreq, 'SUM')
    end if
    ...
    if (on_root) then
	  ...
	  if (eval_polar) then
		write (stdout, '(/,1x,a)') &
            '----------------------------------------------------------'
        write (stdout, '(1x,a)') &
            'Output files for polarizability in imaginary frequency:   '
        write (stdout, '(1x,a)') &
            '----------------------------------------------------------'
		fac1 = elem_charge_SI/(cell_volume)
		polar_list = fac1*polar_list
		! convert alpha to Farad/Angstrom
		do i = 1, 3
		  do j = 1, 3
			file_name = trim(seedname)//'-polarizability_'// &
				        achar(119 + i)//achar(119 + j)//'.dat'
			file_name = trim(file_name)
			file_unit = io_file_unit()
			open (file_unit, FILE=file_name, STATUS='UNKNOWN', FORM='FORMATTED')
			write (stdout, '(/,3x,a)') '* '//file_name
			do ifreq = 1, kubo_nfreq
			  write (file_unit, '(1E16.8, 2E16.8)') &
			        real(kubo_freq_list(ifreq), dp), polar_list(i,j,ifreq)
			enddo
		    close (file_unit)
		  enddo		  
		enddo
...
!             delta=delta/eta_smr
  ...
  subroutine berry_get_polar_klist(kpt, polar_k_list)
    !====================================================================!
    !  vdW-WanMBD method                                             !
    !  Contribution from point k to the polarization                     !
    ...
    use w90_io, only: io_error, stdout, io_file_unit, seedname, &
      io_stopwatch, io_date ! vdW-WanMBD method: modify to calculate polarizability alpha
    ! Arguments
    !
    real(kind=dp), intent(in)                           :: kpt(3)
	complex(kind=dp), intent(out), dimension(:, :, :)   :: polar_k_list
	real(kind=dp)  :: freqfactor_mn
    ...
    allocate (eig_da(num_wann, 3))
   	...
   	! initialize polarizability at point k
	polar_k_list = cmplx_0
	...
	! vdW-WanMBD method: modify to calculate contribution of alpha at k point  
    ...
    do n = 1, num_wann
	  do m = 1, num_wann
		...
		! freqfactor_mn = hbaromega/(hbaromega_mn^2 + hbaromega^2)
		do ifreq = 1, kubo_nfreq
		   freqfactor_mn = (occ_fac*(eig(n)-eig(m)))/((eig(n)-eig(m))**2 + omega(ifreq)**2)
		   call ZGERC(3, 3, freqfactor_mn, r_mn(1:3), 1, r_mn(1:3), 1, polar_k_list(1:3,1:3,ifreq), 3)
		   !do a = 1, 3
		      !do b = 1, 3
			     !polar_k_list(a,b,ifreq) = polar_k_list(a,b,ifreq) + freqfactor_mn * r_mn(a) * conjg(r_mn(b))
			  !enddo
		   !enddo
		enddo ! calculate contribution of alpha at k point
      enddo ! bands
    enddo ! bands
    !--------------------------------
      
  end subroutine berry_get_polar_klist
